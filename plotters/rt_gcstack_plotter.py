"""
RT GCStack CPI Stack plotter - Stacked bar charts for RT performance breakdown
"""
...
class RTGCStackPlotter(Plotter):
    ...
    def plot(
        ...
        """
        Create grouped stacked bar chart for RT GCStack CPI breakdown

        Args:
            data: DataFrame with columns: simulation, scene, rtgcstack_base, rtgcstack_travdata, ...
        ...
        """
        # Copy dataframe and rename columns to remove rtgcstack_ prefix
        df = data.copy()

        # Rename rtgcstack_ prefixed columns to unprefixed versions
        rename_map = {}
        for col in df.columns:
            if col.startswith('rtgcstack_'):
                new_col = col.replace('rtgcstack_', '')
    ...
    def plot_comparison(
        ...
        Args:
            data: DataFrame with RT GCStack data
        ...
        # Plot baseline
        self._plot_single_sim(axes[0], df, baseline_sim, f"{baseline_sim}\n(Baseline)")

        # Plot other simulations
        for idx, sim in enumerate(other_sims):
            self._plot_single_sim(axes[idx + 1], df, sim, sim)
...
def plot_rt_gcstack(
    ...
    **kwargs
    ...
    """
    Quick RT GCStack plotting function

    Args:
        data: DataFrame with RT GCStack data
        output_file: Path to save plot
        title: Plot title
        **kwargs: Additional arguments for RTGCStackPlotter.plot()

    Example:
        plot_rt_gcstack(df, Path('plots/rt_gcstack.png'), title='My Analysis')
    """
    # Ensure plots directory exists
    output_file.parent.mkdir(parents=True, exist_ok=True)

    plotter = RTGCStackPlotter()
