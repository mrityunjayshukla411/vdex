"""
GCStack CPI Stack extractor - extracts performance breakdown statistics
"""
...
class GCStackExtractor:
    """Extract GCStack CPI Stack statistics"""

    def __init__(self, name: str = "gcstack_experiment"):
        self._name = name
        self._simulations: List[Simulation] = []
        self._scenes: List[Scene] = []
        self._parser = GCStackParser()
        self._repository: Repository = CachedRepository(ParquetRepository())
        self._dataset: Optional[Dataset] = None

    def add_simulation(self, simulation: Simulation) -> 'GCStackExtractor':
        """Add simulation (fluent)"""
        self._simulations.append(simulation)
        return self

    def add_scene(self, scene: Scene) -> 'GCStackExtractor':
        """Add scene (fluent)"""
        self._scenes.append(scene)
        return self

    def extract(self) -> 'GCStackExtractor':
        """Extract GCStack CPI Stack data"""
        print(f"\n{'='*60}")
        print(f"Extracting GCStack CPI Stack: {self._name}")
        print(f"{'='*60}")
            ...
            for scene in self._scenes:
                try:
                    ...
                    if not df.empty:
                        ...
                        print(f"  ✗ {scene.name}: No GCStack data")
        ...
        if all_dfs:
            ...
            print(f"\n{'='*60}")
            print(f"✗ No GCStack data extracted")
            print(f"{'='*60}\n")
    ...
    def _save_dataframe(self, df):
        ...
        file_path = exp_dir / "gcstack_cpi.parquet"
    ...
    def save(self, also_csv: bool = False) -> 'GCStackExtractor':
        """
        ...
        Returns:
            self (fluent)
        """
        file_path = self._repository.data_dir / self._name / "gcstack_cpi.parquet"
        ...
        if also_csv:
            csv_path = file_path.parent / "gcstack_cpi.csv"
    ...
    def load(self) -> 'GCStackExtractor':
        """
